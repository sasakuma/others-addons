...

{
    ...
    'summary': 'Display attractive range sliders for number selection in '
               'form view',
    'description': """
...
Use options as illustrated in the examples to define additional options for
the widget
...
* range: 'min' for Fixed Minimum, 'max' for Fixed Maximum,
        ...
        Note: For 'range' values, the values are stored in the database in the
        format 'lower value,upper value'.
* text_before: Define the string to be displayed before the value e.g. '$',
'USD' etc.
* separator: Define the separator string e.g. ' - ', ' to ' etc.
* text_after: Define the string to be displayed after the value e.g. 'sq. ft.'
etc.
...
<group>
    <field name="bedrooms" string="Bedrooms"
            ...
            "{'color':'black','max_val':7,'min_val':1}" />
    <field name="area"
            string="Built-up Area"
            widget="slider"
            options="{'step':50,'color':'red','range':'min','text_after':
            ' sq. ft.','max_val':10000}" />
    <field name="price"
            string="Price Range"  widget="slider"
            options="{'color':'purple','range':'range','text_before':
            '$ ','min_val':10000,'separator':'
            - $ ','max_val':80000,'step':1000}" />
</group>
<group>
    <field name="floor" string="Floor No." widget="slider" options=
    "{'orientation':'vertical','range':'max','max_val':100}"/>
</group>

Notes
--------------

Before setting the widget, avoid garbage values in the field to avoid
errors.""",
