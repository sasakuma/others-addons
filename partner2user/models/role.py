...
class ResUsersRole(models.Model):
    ...
    def copy(self, default=None):
        ...
        group_dict = self._prepare_dict_to_create_backup_record(self.group_id)
        group_dict['name'] = copy_name
        group_rec = self.env['res.groups'].create(group_dict)

        role_dict = self._prepare_dict_to_create_backup_record(self)
        role_dict['name'] = copy_name
        role_dict['group_id'] = group_rec.id
        role_dict['line_ids'] = False


        # default['group_id'] = self.env['res.groups'].create(group_dict).id
        # copy_record = super(ResUsersRole, self).copy(default=default)
