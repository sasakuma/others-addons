...
from odoo import models, fields, api, http, request, etree
...
class Osmer(models.Model):
    ...
    def open_map(self):
        ...
        map_url = request.httprequest.host_url + "osmer_map?secret=" + str(
            rand_number)
...
class MyController(http.Controller):
    ...
    def some_code(self, **kwargs):
        ...
        my_event = request.env['event.event'].sudo().search(
            [('osmer_secret', '=', values['secret'])], limit=1)[0]
        ...
        for reg in my_event.registration_ids:
            ...
            if len(my_elements) != 0:
                ...
                end_string += 'var lonLat = new OpenLayers.LonLat(' + str(
                    my_long) + ',' + str(my_lat) + ')' + "\n"
                end_string += '          .transform(' + "\n"
                end_string += '            new OpenLayers.Projection(' \
                              '"EPSG:4326"), // transform from WGS 1984' + "\n"
                end_string += '            map.getProjectionObject() // ' \
        ...
        my_string += '<script src="http://www.openlayers.org/api/' \
                     'OpenLayers.js"></script>' + "\n"
        my_string += '<script>' + "\n"
        my_string += '    map = new OpenLayers.Map("mapdiv");' + "\n"
        my_string += '    map.addLayer(new OpenLayers.Layer.OSM());' + "\n"

        my_string += '    var lonLatAverage = new OpenLayers.LonLat(' + str(
            long_average) + ',' + str(lat_average) + ')' + "\n"
        my_string += '          .transform(' + "\n"
        my_string += '            new OpenLayers.Projection("EPSG:4326"),' \
                     ' // transform from WGS 1984' + "\n"
        my_string += '            map.getProjectionObject() // ' \
                     'to Spherical Mercator Projection' + "\n"
        my_string += '          );' + "\n"

        my_string += '    var zoom=9;' + "\n"

        my_string += '    var markers = new OpenLayers.Layer.Markers(' \
                     ' "Markers" );' + "\n"
